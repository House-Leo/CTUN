...
class VideoTestDataset(data.Dataset):
    ...
    def __init__(self, opt):
        ...
        if opt['name'].lower() in ['vid4', 'reds4', 'redsofficial', 'vc4']:
...
#         self.lqs_path, self.gts_path = [], []

#         with open(meta_info_file, 'r') as fin:
#             for line in fin:
#                 scene, frame_num, _ = line.strip().split(' ')
#                 image_path_list.extend([osp.join(scene, f"{i:08d}") for i in range(int(frame_num))])
#                 self.folders.append(scene)
#                 self.lqs_path.append(osp.join(self.lq_root, scene))
#                 self.gts_path.append(osp.join(self.gt_root, scene))

#         if self.cache_data:
#             logger.info(f'Cache TestDataset ...')
#             self.lqs_data = [read_img_seq(osp.join(self.lq_root, scene)) for  scene in self.folders]
#             self.gts_data = [read_img_seq(osp.join(self.gt_root, scene)) for  scene in self.folders]
...
#         else:
#             data_lqs = read_img_seq(osp.join(self.lq_root, scene))
#             data_gts = read_img_seq(osp.join(self.gt_root, scene))
#         return {
#             'lq': imgs_lq,
#             'gt': imgs_gt,
#             'folder': scene,
