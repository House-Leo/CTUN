...
class VideoRecurrentModel(VideoBaseModel):
            ...
            if i < num_folders:
                for idx in range(visuals['result'].size(1)):
                    ...
                    if save_img:
                        ...
                        # result_img = torch.clamp(result_img, min=0.0)
                        # # result_img = np.array(result_img[:3, :, :].permute(1, 2, 0).detach().cpu() * 255.)
                        # result_img = np.array(result_img[:3, :, :].permute(1, 2, 0).detach().cpu())
                        # print( result_img.shape)
                        # # cv2.imwrite(img_path, result_img[:,:,::-1])
    ...
    def test(self):
        ...
        with torch.no_grad():
            if self.opt['val'].get('chunk', False):
                lq1, lq2, lq3 = self.lq.chunk(3, dim = 1)
                out1 = self.net_g(lq1).detach().cpu()
                out2 = self.net_g(lq2).detach().cpu()
                out3 = self.net_g(lq3).detach().cpu()
                self.output = torch.cat([out1, out2, out3], dim = 1)
            else:
